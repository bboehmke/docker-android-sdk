...
RUN curl -sSL https://services.gradle.org/distributions/gradle-2.6-bin.zip -o /tmp/gradle-bin.zip && \
    curl -sSL http://dl.google.com/android/android-sdk_r24.3.4-linux.tgz | tar xz -C /usr/local && \
...
# set path
ENV PATH $PATH:${ANDROID_HOME}/tools:$ANDROID_HOME/platform-tools:/usr/local/gradle-2.6/bin
